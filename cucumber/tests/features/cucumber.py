"""Cucumber.py is the Gherkin Test for Shopping Cart

    __author__ = "SOK HENG LIM - 260581435"
    __author__ = "Zhi Peng Liu - 260505428"
"""
import os
...
DRIVER_DIR = os.path.join(BASE_DIR, "chrome_driver", driver_name)  # full path to chrome river
...
def verify_product(step, expected_result):
    actual_result = driver.find_element_by_xpath(
        '//*[@id="161728962861_title"]/a').text  # find the blender on cart page by its product id
...
def display_error(step, expected_error):
    ...
    assert_equals(expected_error,
                  actual_error)  # check error message exists and is the expected message given by step definition
